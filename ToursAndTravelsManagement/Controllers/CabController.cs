...
namespace ToursAndTravelsManagement.Controllers
{
    ...
    public class CabController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // Get featured cabs
            var featuredCabs = await _context.Cabs
                .Include(c => c.CabRoutePrices)
                .Where(c => c.IsActive &&
    ...
    crp.Route != null) 
        ...
        public async Task<IActionResult> Book(CabBookingViewModel model)
        {
            ...
            {
                // Online / UPI
                return RedirectToAction("Pay", new { bookingId = booking.CabBookingId });
            }
        ...
        {
            var booking = await _context.CabBookings
                .FirstOrDefaultAsync(b => b.CabBookingId == bookingId);

            if (booking == null)
                return NotFound();

            // Get UPI settings (only one row expected)
            var paymentSetting = await _context.PaymentSettings.FirstOrDefaultAsync();

            if (paymentSetting == null)
            {
                TempData["Error"] = "Online payment is currently unavailable. Please contact admin.";
                return RedirectToAction("BookingConfirmation", new { id = bookingId });
            }

            // Create UPI URL
            string upiUrl =
                $"upi://pay?pa={paymentSetting.UpiId}" +
                $"&pn={paymentSetting.UpiName}" +
                $"&am={booking.TotalPrice}" +
                $"&cu=INR" +
                $"&tn=CabBooking-{booking.CabBookingId}";
            ...
            ViewBag.QrCode = Convert.ToBase64String(qrCode.GetGraphic(20));
        ...
        //    string upiUrl =
        //        $"upi://pay?pa={paymentSetting.UpiId}&pn={paymentSetting.UpiName}&am={booking.TotalPrice}&cu=INR&tn=CabBooking";
        ...
        public async Task<IActionResult> ConfirmPayment(int bookingId, string utr)
        {
            ...
            if (booking.Cab != null)
            {
                booking.Cab.Status = CabStatus.Booked;
            }
