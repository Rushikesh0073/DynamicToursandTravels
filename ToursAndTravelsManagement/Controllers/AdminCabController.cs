...
namespace ToursAndTravelsManagement.Controllers
{
    ...
    public class AdminCabController : Controller
    {
        ...
        //}
         ...
         [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> UpdateCabStatus(int id, CabStatus status)
    {
        var cab = await _context.Cabs.FindAsync(id);
        if (cab != null)
        {
            ...
            _context.Cabs.Update(cab);
            await _context.SaveChangesAsync();
            TempData["Success"] = $"Cab status updated to {status} successfully!";
        }
        return RedirectToAction("ManageCabs");
    }
        ...
        // below code is working correctly but in above code all all cab forthat location
        //public async Task<IActionResult> ManageCabPrices()
        //{
        //    var cabPrices = await _context.CabRoutePrices
        //        .AsNoTracking()                             // performance + read-only
        ...
        // Add these methods to your AdminCabController
        ...
        [HttpGet]
        public async Task<IActionResult> GetCabDetails(int id)
        {
            var cab = await _context.Cabs.FindAsync(id);
            if (cab == null)
            ...
            {
                success = true,
                basePricePerKM = cab.BasePricePerKM,
                name = cab.Name,
        ...
        [HttpPost]
        public async Task<IActionResult> CheckExistingPrice(int cabId, int routeId, JourneyType journeyType)
        {
            var existingPrice = await _context.CabRoutePrices
                .FirstOrDefaultAsync(crp =>
                    crp.CabId == cabId &&
                    crp.RouteId == routeId &&
                    crp.JourneyType == journeyType &&
                    crp.IsActive);

            return Json(new { exists = existingPrice != null });
        }

        [HttpPost]
        public async Task<IActionResult> GetExistingPrice(int cabId, int routeId)
        {
            // Get any existing price for similar routes (same from/to cities)
            var cab = await _context.Cabs.FindAsync(cabId);
            if (cab == null)
                return Json(new { success = false });

            // Get first existing price for this cab on any route
            var existingPrice = await _context.CabRoutePrices
                .Include(crp => crp.Route)
                .Where(crp => crp.CabId == cabId && crp.IsActive)
                .FirstOrDefaultAsync();
            ...
            return Json(new
            {
                ...
                price = new
                {
                    includedServices = existingPrice.IncludedServices,
                    excludedServices = existingPrice.ExcludedServices,
                    termsAndConditions = existingPrice.TermsAndConditions,
        ...
        public async Task<IActionResult> CreateCabPrice()
        {
            try
            {
                ...
                // Get active routes with city information
                var routes = await _context.Routes
                    .Include(r => r.FromCity)
                    .Include(r => r.ToCity)
                    .Where(r => r.IsActive)
                ...
                // Create ViewBag items for cabs
                ViewBag.Cabs = cabs.Select(c => new SelectListItem
                {
                    Value = c.CabId.ToString(),
                    Text = $"{c.Name} ({c.Type} - {c.SeatingCapacity} seats)"
                }).ToList();

                // Create ViewBag items for routes
                ViewBag.Routes = routes.Select(r => new SelectListItem
                {
                    Value = r.RouteId.ToString(),
                    Text = $"{r.FromCity.Name} → {r.ToCity.Name} ({r.Distance} km)"
                }).ToList();

                // FIXED: Create JourneyTypes dropdown - IMPORTANT: Use integer values that match your enum
                ViewBag.JourneyTypes = new List<SelectListItem>
        {
            new SelectListItem { Value = "1", Text = "One Way" },
            new SelectListItem { Value = "2", Text = "Outstation" },
            new SelectListItem { Value = "3", Text = "Local" },
            new SelectListItem { Value = "4", Text = "Transfer" }
        };

                return View(new CabRoutePrice
                {
                    IsAvailable = true,
            ...
            {
                _logger.LogError(ex, "Error loading create cab price page");
        ...
        public async Task<IActionResult> CreateCabPrice(CabRoutePrice cabPrice)
        {
            try
            {
                ...
                _logger.LogInformation($"Creating cab price - CabId: {cabPrice.CabId}, RouteId: {cabPrice.RouteId}, JourneyType: {cabPrice.JourneyType}, Price: {cabPrice.Price}");
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (existingPrice != null)
                    {
                        // Allow multiple prices but warn
                        TempData["Warning"] = "A price already exists for this cab, route, and journey type combination. You can still save this as a different entry.";
                    }
                    ...
                    // No need to check HasValue since IsAvailable is non-nullable bool
                    // It will always have a value (default is false if not set)
                ...
                else
                {
                    ...
                    // If validation fails, reload ViewBag data
            ...
            catch (Exception ex)
            {
                ...
                TempData["Error"] = $"An error occurred while creating cab price: {ex.Message}";
                await LoadCreateCabPriceViewData();
                return View(cabPrice);
            }
        }

        // below code work correctly just uaded for all car servive bove code 
        ...
        private async Task LoadCreateCabPriceViewData()
        {
            ...
            // FIXED: Create JourneyTypes dropdown
            ViewBag.JourneyTypes = new List<SelectListItem>
    {
        new SelectListItem { Value = "1", Text = "One Way" },
        new SelectListItem { Value = "2", Text = "Outstation" },
            ...
            var validTransitions = new Dictionary<CabBookingStatus, List<CabBookingStatus>>
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ConfirmPayment(int bookingId, string utr)
        {
            var booking = await _context.CabBookings.FindAsync(bookingId);
            ...
            return RedirectToAction("PaymentPending");
        }
        ...
        public async Task<IActionResult> ApprovePayment(int id)
        {
            var payment = await _context.Payments
                .Include(p => p.CabBooking)
                .FirstAsync(p => p.PaymentId == id);

            payment.Status = PaymentStatus.Completed;
            payment.CabBooking.Status = CabBookingStatus.Confirmed;

            await _context.SaveChangesAsync();

            return RedirectToAction("GenerateInvoice", new { bookingId = payment.CabBookingId });
        }
